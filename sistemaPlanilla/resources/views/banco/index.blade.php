...
<a class="modal-with-form btn btn-primary mb-3" href="#modalCrearProfesion" id="btnCrear">
    Agregar Banco <i class="fa fa-plus-circle" aria-hidden="true"></i></a>
<section class="panel">
            ...
            @foreach ($bancos as $banco)
                <tr class="gradeX">
                    ...
                    <td class="text-center">
                        <a class="modal-with-form btn btn-primary editar" id="{{'editar'.$banco -> idbanco }}" href="#modalEditarProfesion" data-id="{{$banco -> idbanco }}">
                        <i class="fa fa-pencil" aria-hidden="true"></i>
                        </a>
                        <form id="{{ 'formulario-prueba'.$banco -> idbanco  }}" class="btn btn-danger p-0" method="post" action="{{ url('/banco/'.$banco -> idbanco ) }}">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                            <button class="btn btn-danger border-0" type="submit" onclick="presionar('{{ $banco -> idbanco }}', '{{ $banco -> nombrebanco }}','el banco')">
...
<div class="modal-block modal-block-primary mfp-hide" id="modalCrearProfesion" tabindex="-1" role="dialog" aria-labelledby="modalCrearProfesion" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <form id="demo-form" class="form-horizontal" action="{{ url('/banco') }}" method="post">
                {{ csrf_field() }}
...
<div class="modal-block modal-block-primary mfp-hide" id="modalEditarProfesion" tabindex="-1" role="dialog" aria-labelledby="modalEditarProfesion" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <form id="editar-form" class="form-horizontal" action="/banco" method="post">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        table.on('click', '.editar', function() {
            ...
            $('#nombreBanco').val(data[1]);

            $('#editar-form').attr('action', '/banco/' + id);
