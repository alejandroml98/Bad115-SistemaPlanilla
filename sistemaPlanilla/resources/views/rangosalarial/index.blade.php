...
<a class="modal-with-form btn btn-primary mb-3" href="#modalCrearRangoS" id="btnCrear">
    Agregar Rango Salarial <i class="fa fa-plus-circle" aria-hidden="true"></i></a>
<section class="panel">
    <header class="panel-heading">
        <h2 class="panel-title">Rangos Registrados</h2>
    </header>
            ...
            <tbody>
                ...
                <tr class="gradeX">
                    ...
                    <td class="text-center">
                        <a class="modal-with-form btn btn-primary editar" id="{{'editar'.$rangoSalarial -> idrangosalarial}}" href="#modalEditarRangoS" data-id="{{$rangoSalarial -> idrangosalarial}}">
                            <i class="fa fa-pencil" aria-hidden="true"></i>
                        </a>
                        <form id="{{ 'formulario-prueba'.$rangoSalarial -> idrangosalarial }}" class="btn btn-danger p-0" method="post" action="{{ url('/rangosalarial/'.$rangoSalarial -> idrangosalarial) }}">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                            <button class="btn btn-danger border-0" type="submit" onclick="presionar('{{ $rangoSalarial -> idrangosalarial}}', '{{ $rangoSalarial -> salariominimo }} hasta {{ $rangoSalarial -> salariomaximo }}',' El rango ')">
...
<div class="modal-block modal-block-primary mfp-hide" id="modalCrearRangoS" tabindex="-1" role="dialog" aria-labelledby="modalCrearRangoS" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <form id="demo-form" class="form-horizontal" action="{{ url('/rangosalarial') }}" method="post">
                {{ csrf_field() }}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        table.on('click', '.editar', function() {
            ...
            $('#salarioMinimo').val(data[1]);
            $('#salarioMaximo').val(data[2]);

            $('#editar-form').attr('action', '/rangosalarial/' + id);
